...
def send_notification(project):
    try:
        ...
        # Add user info with Solscan link
        user_info = project.get("user", {})
        creator = user_info.get("username") or user_info.get("displayName", "Unknown")
        creator_wallet = user_info.get("solanaWallet", "")
        
        if creator_wallet:
            solscan_url = f"https://solscan.io/account/{creator_wallet}"
            embed["fields"].append({
                "name": "Created By",
                "value": f"[{creator}]({solscan_url})",
                "inline": True
            })
        else:
            embed["fields"].append({
